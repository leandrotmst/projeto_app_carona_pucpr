<?php
    ...
    if(isset($_GET['id_carona'])){
        ...
        destino=?, data_partida=?, hora_partida=?, id_veiculo=?, vagas=?, distancia=?, 
        tempo_estimado=? WHERE id_carona=?");
        $stmt->bind_param("issssiidsi",$id_usuario, $origem, $destino, $data_partida, 
        $hora_partida, $id_veiculo, $vagas, $distancia, $tempo_estimado, $_GET['id_carona']);
        $stmt->execute();
